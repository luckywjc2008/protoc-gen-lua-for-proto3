...
local function _AddMergeFromStringMethod(message_descriptor, message_meta)
    ...
    local _internal_parse = function(self, buffer, pos, pend,cmdId)
        message_meta._member._Modified(self)
        ...
        while pos ~= pend do
            ...
            if field_decoder == nil then
                new_pos = SkipField(buffer, new_pos, pend, tag_bytes,cmdId)
                if new_pos == -1 then
                    return pos
                end
                pos = new_pos
                ...
                pos = field_decoder(buffer, new_pos, pend, self, field_dict,cmdId)
            end
        end
        return pos
    end
    ...
    local merge_from_string = function(self, serialized,cmdId)
        local length = #serialized
        cmdId = cmdId or 0

        if _internal_parse(self, serialized, 0, length,cmdId) ~= length then
            error('Unexpected end-group tag.'..cmdId)
        end
        return length 
    end
    message_meta._member.MergeFromString = merge_from_string

    message_meta._member.ParseFromString = function(self, serialized,cmdId)
        message_meta._member.Clear(self)
        merge_from_string(self, serialized,cmdId)
    end
end
