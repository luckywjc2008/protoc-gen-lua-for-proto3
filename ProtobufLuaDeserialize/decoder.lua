...
function MessageDecoder(field_number, is_repeated, is_packed, key, new_default,cmdId)
    local DecodeVarint = _DecodeVarint
    local sub = string.sub
    assert(not is_packed)
    if is_repeated then
        local tag_bytes = encoder.TagBytes(field_number, wire_format.WIRETYPE_LENGTH_DELIMITED)
        local tag_len = #tag_bytes
        return function (buffer, pos, pend, message, field_dict,cmdId)
            local value = field_dict[key]
            if value == nil then
                value = new_default(message)
                field_dict[key] = value
            ...
            while 1 do
                ...
                if value:add():_InternalParse(buffer, pos, new_pos,cmdId) ~= new_pos then
        ...
        return function (buffer, pos, pend, message, field_dict,cmdId)
            ...
            if value:_InternalParse(buffer, pos, new_pos,cmdId) ~= new_pos then
                error('Unexpected end-group tag.')
            end
            return new_pos
        end
...
function _SkipVarint(buffer, pos, pend,cmdId)
    local value
...
function _SkipFixed64(buffer, pos, pend,cmdId)
    pos = pos + 8
    if pos > pend then 
        cmdId = cmdId or 0
        error('Truncated message.'..cmdId)
    end
    return pos
end

function _SkipLengthDelimited(buffer, pos, pend,cmdId)
    local size
...
function _SkipFixed32(buffer, pos, pend,cmdId)
    pos = pos + 4
    if pos > pend then
        cmdId = cmdId or 0
        error('Truncated message.'..cmdId)
...
function _RaiseInvalidWireType(buffer, pos, pend,cmdId)
    cmdId = cmdId or 0
    error('Tag had invalid wire type.'..cmdId)
end
...
--    wiretype_mask = wire_format.TAG_TYPE_MASK
    ...
    return function (buffer, pos, pend, tag_bytes,cmdId)
        local wire_type = ord(sub(tag_bytes, 1, 1)) % 8 + 1
        return WIRETYPE_TO_SKIPPER[wire_type](buffer, pos, pend,cmdId)
    end
end
